...
class fmha_bwd_v3_kernel
{
    ...
    fmha_bwd_v3_kernel(const char *name, const char *hsaco)
    {
        ...
        const char* env_dir = std::getenv("AITER_ASM_DIR");
        if (!env_dir) {
            throw std::runtime_error("AITER_ASM_DIR environment variable is not set. e.g. export AITER_ASM_DIR=/path/to/aiter/hsa/gfx950/");
        }
        std::string AITER_ASM_DIR = std::string(env_dir) + "fmha_v3_bwd/";
